...
map_taiwan = folium.Map(location=[23.6978, 120.9605], zoom_start=8)
marker_cluster = MarkerCluster().add_to(map_taiwan)

scenic_spots = {
    '士林夜市': [25.088944, 121.524583],
    '九份': [25.109917, 121.845222],
    '大稻埕': [25.0564, 121.50761],
...
for idx, (spot, coords) in enumerate(scenic_spots.items()):
    ...
    <div>
        ...
        <br><br>
        <button onclick="sendData()">加入我的行程</button><br><br>
        <button onclick="deletData()">取消我的行程</button>
    </div>
    <script>
    ...
    folium.Marker(
        location=coords,
        popup=popup,
        icon=folium.features.CustomIcon(picture_url, icon_size=(30, 30))
    ).add_to(map_taiwan)
...
<button class="view-button" onclick="viewSpots()">查看我的行程</button>
<button class="clear-button" onclick="clearSpots()">清除我的行程</button>
<button class="post-button" onclick="postSpots()">送出我的行程</button>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        window.addEventListener('beforeunload', function (e) {
            var url = "http://127.0.0.1:5000/clear_my_trip";
            fetch(url)
    ...
    function postSpots() {
        var url = "http://127.0.0.1:5000/post_my_trip";
        fetch(url)
            .then(response => {
                if (!response.ok) {
                    throw new Error('Network response was not ok');
                }
                alert("行程傳送成功！"); // 在確認請求成功後顯示提示
                return response.json(); // 繼續處理 JSON 數據
            })
            .then(data => {
                var spotsContainer = document.getElementById("spotsContainer");
                spotsContainer.innerHTML = "";
...
map_taiwan.save(r"map_taiwan.html")
